...
export class Controller {
    ...
    initBotones() {
        ...
        const SVG_SONIDO_ON = '<svg class="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polygon points="11 5 6 9 2 9 2 15 6 15 11 19 11 5"/><path d="M15.54 8.46a5 5 0 0 1 0 7.07"/><path d="M19.07 4.93a10 10 0 0 1 0 14.14"/></svg>';
        const SVG_SONIDO_OFF = '<svg class="w-4 h-4 opacity-50 text-slate-400" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polygon points="11 5 6 9 2 9 2 15 6 15 11 19 11 5"/><line x1="23" y1="9" x2="17" y2="15"/><line x1="17" y1="9" x2="23" y2="15"/></svg>';

        const actualizarIconosSonido = () => {
            const habilitado = SoundEffects.estaHabilitado();
            const svg = habilitado ? SVG_SONIDO_ON : SVG_SONIDO_OFF;
            const label = habilitado ? 'Silenciar sonido' : 'Activar sonido';
            if (btnSonido) {
                btnSonido.innerHTML = svg;
                btnSonido.setAttribute('aria-label', label);
            }
            if (btnSonidoDesk) {
                const iconHolder = btnSonidoDesk.querySelector('.icon-holder');
                if (iconHolder) {
                    iconHolder.innerHTML = svg;
                } else {
                    btnSonidoDesk.innerHTML = svg;
                }
