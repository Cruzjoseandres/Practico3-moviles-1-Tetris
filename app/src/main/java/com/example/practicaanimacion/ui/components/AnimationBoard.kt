...
class AnimationBoard(context: Context?, attrs: AttributeSet?) :
    ...
    private var cuadrado = Square(100f, 100f, android.graphics.Color.RED).apply {
    ...
    override fun onDraw(canvas: Canvas) {
        super.onDraw(canvas)
        cuadrado.changeScreenSize(width, height)
        cuadrado.draw(canvas, paint)
    }

    fun moveUp() {
        cuadrado.animate(Direction.UP)
    }

    fun moveDown() {
        cuadrado.animate(Direction.DOWN)
    }

    fun moveLeft() {
        cuadrado.animate(Direction.LEFT)
    }

    fun moveRight() {
        cuadrado.animate(Direction.RIGHT)
    }
